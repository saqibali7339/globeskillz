...
(() => {
  ...
  // Theme
  $$('#tweakThemes .tweaks__swatch').forEach(sw => {
    sw.addEventListener('click', () => {
      $$('#tweakThemes .tweaks__swatch').forEach(s => s.classList.remove('active'));
  ...
  // Hero style
  const heroGlobeSvg = $('.hero__globe');
  const wordmark = $('.hero__wordmark');
  $$('#tweakHero .tweaks__radio').forEach(r => {
    r.addEventListener('click', () => {
      $$('#tweakHero .tweaks__radio').forEach(x => x.classList.remove('active'));
      r.classList.add('active');
      const mode = r.dataset.hero;
      if (!heroGlobeSvg) return;
      heroGlobeSvg.style.opacity = mode === 'kinetic' ? '0.15' : '1';
      if (mode === 'particle') buildParticleGlobe();
      else if (mode === 'wireframe') restoreWireframe();
      else restoreWireframe();
      if (wordmark) wordmark.style.transform = mode === 'kinetic' ? 'scale(1.2)' : '';
    });
  });

  let originalGlobeHTML = null;
  function saveOriginalGlobe() {
    if (!originalGlobeHTML && heroGlobeSvg) originalGlobeHTML = heroGlobeSvg.innerHTML;
  }
  saveOriginalGlobe();
  function restoreWireframe() { if (heroGlobeSvg && originalGlobeHTML) heroGlobeSvg.innerHTML = originalGlobeHTML; }
  function buildParticleGlobe() {
    if (!heroGlobeSvg) return;
    const w = 800, cx = 400, cy = 400, r = 340;
    let dots = '';
    for (let lat = -80; lat <= 80; lat += 10) {
      const y = cy + (lat / 90) * r;
      const rr = Math.cos(lat * Math.PI / 180) * r;
      const count = Math.max(6, Math.round(rr / 12));
      for (let i = 0; i < count; i++) {
        const a = (i / count) * Math.PI * 2;
        const x = cx + Math.cos(a) * rr;
        const size = 1.2 + Math.random() * 1.6;
        dots += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${size.toFixed(1)}" fill="url(#hgradbold)" opacity="${(0.4 + Math.random() * 0.5).toFixed(2)}"/>`;
      }
    }
    heroGlobeSvg.innerHTML = `<svg viewBox="0 0 800 800"><g class="hero__globe-rotate" style="transform-origin:400px 400px">${dots}</g></svg>`;
  }

  // Intensity
  const intensity = $('#tweakIntensity');
  const intensityVal = $('#tweakIntensityVal');
  intensity?.addEventListener('input', (e) => {
    const v = parseInt(e.target.value, 10);
    document.documentElement.style.setProperty('--intensity', (v / 10).toString());
    if (intensityVal) intensityVal.textContent = v + ' / 10';
    if (v < 3) document.body.classList.add('reduce-motion');
    else document.body.classList.remove('reduce-motion');
  });

  // Grain toggle
  const grain = $('#tweakGrain');
  grain?.addEventListener('click', () => {
    grain.classList.toggle('on');
    document.body.classList.toggle('no-grain', !grain.classList.contains('on'));
  });
  // Cursor toggle
  const cursorTog = $('#tweakCursor');
  cursorTog?.addEventListener('click', () => {
    cursorTog.classList.toggle('on');
    document.body.classList.toggle('no-cursor', !cursorTog.classList.contains('on'));
  });
})();
